    ...
    <div v-cloak id='input' class='initial' v-bind:class="{ active: isActiveMid}">
        <transition-group :name="transitionName">
            ...
            <div class='entry' :key="1" v-if='step === 1'>
                ...
                <form @submit.prevent="changeStep('next')">
                    <input type='number' min='1' max='10' class='nameEntry numEntry' maxlength="50" v-model="many" required><br>
                <h2>And how seconds should it show for?</h2>
                    <input type='number' min='1' max='20' class='nameEntry numEntry' maxlength="50" v-model="timer" required><br>
            ...
            <h2>Your Message?</h2>
                <form @submit.prevent="changeStep('next')">
                    <textarea class='textEntry' maxlength="918" v-model="tape" placeholder='Message' required></textarea><br>
                    <button type="button" v-on:click="changeStep('prev')"><i class="fas fa-step-backward"></i></button>
                    <button type='submit'><i class="fas fa-step-forward"></i></button>
                </form>
            </div>
            ...
            <div class='entry' :key="6" v-if='step === 6'>
                <form @submit.prevent="submit">
                <h2>Codewords?</h2>
            ...
            <div id='letter' :key="7" v-if='step === 7'>
